...
def augment_modalities(text_tensor, audio_tensor, cfg):
    ...
    text_noise_std = cfg.get("text_noise_std", 0.0)

    text_aug = apply_word_dropout(text_tensor, word_dropout)
    if text_noise_std > 0.0 and text_aug is not None:
        text_aug = add_gaussian_noise(text_aug, text_noise_std)
...
def train_and_evaluate(model, train_dataset, valid_dataset, test_dataset,
    ...
    ratio_value = log_context.get("unlabeled_ratio")
    ...
    if apply_consistency:
        base_config.update({
            ...
            "weak_text_noise_std": weak_cfg["text_noise_std"],
