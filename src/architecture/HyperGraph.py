...
class DualHypergraphModule(nn.Module):
    ...
    def forward(self, text_encoded, audio_encoded, record_stats=True):
        ...
        if record_stats:
            with torch.no_grad():
                # Text-to-audio edges: count where sim_t2a >= threshold_text (if threshold provided) else compute Top-K %
                def compute_pct_from_mask(mask, nodes_other):
                    # mask: (B, N_src, N_other) float
                    B, Nsrc, Nother = mask.shape
                    # remove self if same token space (not necessary here since text vs audio)
                ...
                if self.threshold_text is not None:
                    ...
                    # top-K: each text node connects to Kt neighbors
                    Kt = min(self.K_text, La)
