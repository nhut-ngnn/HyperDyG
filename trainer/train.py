...
from src.architecture.HyperDyG import HyperDyG
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train HyperDyG on IEMOCAP or ESD with 5 seeds")
    parser.add_argument("--data_dir", type=str, required=True)
    parser.add_argument("--dataset", type=str, required=True, choices=["IEMOCAP", "ESD"])
    parser.add_argument("--num_classes", type=int, required=True, choices=[4, 5],
    ...
    parser.add_argument("--unlabeled_ratio", type=float, default=0.0,
                        help="Fraction of training data to treat as unlabeled for semi-supervised learning.")
...
def main():
    ...
    auto_split_enabled = auto_split_ratio > 0.0
    ...
    for seed in seeds:
        ...
        common_context = {
            ...
            "project": f"HyperDyG-EmotionRecognition-{args.dataset}-ablation",
            ...
            "pseudo_weight": args.pseudo_weight if use_pseudo else None,
            ...
            "strong_text_noise_std": args.strong_text_noise_std if use_pseudo else None,
        ...
        if use_pseudo and pseudo_available > 0:
            ...
            if unlabeled_dataset is None or len(unlabeled_dataset) == 0:
                ...
                save_path_semi = (
                    f"saved_model/{args.dataset}_{args.num_classes}class_{ratio_name_component}_HyperDyG_seed{seed}_semi.pt"
                )
                unlabeled_cfg = {
    ...
    if args.results_suffix:
        ...
        result_suffix = "HyperDyG_5seeds" if not use_pseudo else "HyperDyG_SemiPseudo_5seeds"
